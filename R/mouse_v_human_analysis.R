...
mouse_l3a_expr = mouseCPMLog[intersect(mouse_human_homologous_genes$mouseGene, rownames(mouseCPMLog)),mouse_l3a_samples] %>% rowMeans()
human_l3a_expr = humanCPMLog[intersect(mouse_human_homologous_genes$humanGene, rownames(humanCPMLog)),human_l3a_samples] %>% rowMeans()

mouse_l3a_expr = mouse_l3a_expr %>% as.data.frame() %>% tibble::rownames_to_column(var = 'mouseGene') 
colnames(mouse_l3a_expr)[2] = 'mouseExpr'

human_l3a_expr = human_l3a_expr %>% as.data.frame() %>% tibble::rownames_to_column(var = 'humanGene') 
colnames(human_l3a_expr)[2] = 'humanExpr'

l3a_agg = merge(mouse_human_homologous_genes, mouse_l3a_expr)
l3a_agg = merge(l3a_agg, human_l3a_expr)
